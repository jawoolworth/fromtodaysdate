...
function findLastWeekday(rule, year) {
  ...
  if (rule.dayOfWeek > lastWeekdayOfMonth) {
    ...
    return new Date(
      year,
      rule.month + 1,
      0 + (rule.dayOfWeek - lastWeekdayOfMonth)
    );
  } else {
...
function findOtherWeekday(rule, year) {
  ...
  if (dayOfWeekDifference >= 0) {
    calculatedDate = new Date(
      year,
      rule.month,
      calculatedDate.getDate() + dayOfWeekDifference
    );
  } else {
    calculatedDate = new Date(
      year,
      rule.month,
      calculatedDate.getDate() + (dayOfWeekDifference + 7)
    );
  }
  ...
  if (rule.plusDays !== null) {
    return new Date(
      year,
      rule.month,
      calculatedDate.getDate() + (rule.weekOfMonth - 1) * 7 + rule.plusDays
    );
  }
  return new Date(
    year,
    rule.month,
    calculatedDate.getDate() + (rule.weekOfMonth - 1) * 7
  );
}
